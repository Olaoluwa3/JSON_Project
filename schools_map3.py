    ...
    if str(school["NCAA"]["NAIA conference number football (IC2020)"]) in division_dict.values():
        try:
            if school["Total price for in-state students living off campus (not with family)  2020-21 (DRVIC2020)"] > 50000:
                total_cost = school["Total price for in-state students living off campus (not with family)  2020-21 (DRVIC2020)"]
                ...
                lons.append(lon)
                lats.append(lat)
                hover_texts.append(name+', $'+ str(total_cost))
                enrollments.append(enrollment)

        except:
            print("no data available for", school["instnm"])
...
my_layout = Layout(title='Schools with total price for in-state students living off campus over $50,000')
fig = {'data':data, 'layout':my_layout}

offline.plot(fig, filename='Map 3 - Total price for in-state students living off campus.html')
