...
#Map 2
#Percent of total enrollment that are Black or African American over 10%
enrollment_rates_BAA = []
lons = []
lats = []
hover_texts = []
enrollments = []
...
for school in school_data:
    if str(school["NCAA"]["NAIA conference number football (IC2020)"]) in division_dict.values():
        if school["Percent of total enrollment that are Black or African American (DRVEF2020)"] > 10:
            enrollment_rate_BAA = school["Percent of total enrollment that are Black or African American (DRVEF2020)"]
            ...
            enrollment_rates_BAA.append(enrollment_rate_BAA)
            lons.append(lon)
            lats.append(lat)
            hover_texts.append(name+', '+ str(enrollment_rate_BAA) +'%')
            enrollments.append(enrollment)



print(len(enrollment_rates_BAA))
...
my_layout = Layout(title='Schools with over 10 percent enrollment of Black or African American students')
fig = {'data':data, 'layout':my_layout}

offline.plot(fig, filename='Map 2 - Black or African American enrollment rate.html')
